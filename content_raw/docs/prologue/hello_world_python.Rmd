...
knitr::opts_knit$set(root.dir = "../../../static/examples/hello_world_py/") 
...
When you call 'viash run', viash parses the [`config.vsh.yaml`](https://viash.io/examples/hello_world_py/config.vsh.yaml) file, which is a meta description of the component written in the yaml serialization language:
...
These arguments are passed on to the **resources**. In this case, there's a single reference to a file named [`script.py`](https://viash.io/examples/hello_world_py/script.py). This file is the 'brain' of the component, it's small Python script which prints out two environment values: `par_input` and `par_greeter`: 
...
Finally, there's a **tests** section to put your test scripts. It's a good practice to write tests and run these every time you update your component and/or the resources. You can read more about writing and running viash tests [on the Testing page](https://viash.io/good_practices/testing/).
