...
Additional options can be set immediately or during a second run of the bootstrap tool using the following syntax:
...
```{bash, echo=FALSE}
./viash_bootstrap --help | tail -n +5 | awk '{$1=$1};1'
```