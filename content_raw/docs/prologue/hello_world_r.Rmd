...
knitr::opts_knit$set(root.dir = "../../../static/examples/hello_world_r/") 
...
When you call 'viash run', viash parses the [`config.vsh.yaml`](https://viash.io/examples/hello_world_r/config.vsh.yaml) file, which is a meta description of the component written in the yaml serialization language:
...
These arguments are passed on to the **resources**. In this case, there's a single reference to a file named [`script.r`](https://viash.io/examples/hello_world_r/script.r). This file is the 'brain' of the component, it's small R script which prints out two environment values: `par_input` and `par_greeter`: 
